...
namespace AOC.Tests
{
    ...
    public class UnitTest
    {
        ...
        {
            if (string.IsNullOrEmpty(scenario.SamplePath))
            {
                return;
            }
            var input = File.OpenText(scenario.SamplePath).ReadToEnd();
            Conf.IsDump = true;
...
7.35.", 0, 0, 3, ExpectedResult = "12345")]
        ...
        public static IEnumerable<TestScenario> GetScenarios()
        {
            ...
            var days = typeof(Day01Puzzle1).Assembly.GetTypes().Where(t => day.IsAssignableFrom(t));
